...
void compileArgument(Object* param) {
  ...
  if(param->paramAttrs->kind == PARAM_REFERENCE){
    if(lookAhead->tokenType == TK_IDENT){     // the argument must be a lvalue
      checkDeclaredLValueIdent(lookAhead->string);  //Check declared
    }
    else{
...
void compileArguments(ObjectNode* paramList) {
  ...
  if(paramList != NULL && lookAhead->tokenType != SB_LPAR) {   
    error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY, currentToken->lineNo, currentToken->colNo);  //paramList is existed but SB_LPAR is not exist.
  }
  switch (lookAhead->tokenType) {
  case SB_LPAR:
    eat(SB_LPAR);
    ...
    while (lookAhead->tokenType == SB_COMMA) {
      ...
      if(paramList != NULL){
        ...
        error(ERR_PARAMETERS_ARGUMENTS_INCONSISTENCY,currentToken->lineNo,currentToken->colNo); //paramList is not exist.
    }
